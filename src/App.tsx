...
export default function App() {
  ...
  return (
    <div className={`min-h-screen bg-black text-white font-sans selection:bg-yellow-500/30 selection:text-yellow-200 ${isRtl ? 'rtl' : 'ltr'}`}>
      ...
      <section className="py-24 bg-black border-t border-white/5 relative overflow-hidden" id="marketing">
        <div className="absolute right-[20rem] top-[10rem] w-[40rem] h-[40rem] bg-[#D4AF37] opacity-[0.02] blur-[120px] rounded-full pointer-events-none" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
            {t.marketing.materials.map((material, i) => {
              const icons = [FileText, FileCheck, Gift, Smartphone];
              const Icon = icons[i % icons.length];
              return (
                <motion.div 
                  key={i}
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: i * 0.1 }}
                  className="p-6 rounded-2xl bg-neutral-900/40 border border-white/5 hover:border-[#D4AF37]/30 transition-all group flex flex-col items-center text-center"
                >
                  <div className="w-16 h-16 rounded-full bg-black border border-white/10 flex items-center justify-center mb-6 group-hover:bg-[#D4AF37]/10 group-hover:border-[#D4AF37]/50 transition-colors">
                    <Icon className="w-8 h-8 text-[#D4AF37]" />
                  </div>
                  <h4 className="text-xl font-bold text-white mb-3">{material.title}</h4>
                  <p className="text-neutral-400 leading-relaxed text-sm">{material.desc}</p>
                </motion.div>
              );
            })}
          </div>

          <div className="bg-neutral-950 rounded-3xl border border-white/10 overflow-hidden relative">
             ...
             <div className="p-8 lg:p-12">
                ...
                >
                  ...
                  <h4 className="text-2xl md:text-3xl font-bold text-white mb-4">{t.marketing.pitchTitle}</h4>
                  <p className="text-neutral-400 leading-relaxed">{t.marketing.pitchSubtitle}</p>
                </motion.div>

                <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto mb-10">
                   {t.marketing.pitchPoints.map((point, i) => (
                      <motion.div 
                        key={i}
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.5, delay: i * 0.1 }}
                        className="flex gap-4 items-start p-4 rounded-xl bg-black/50 border border-white/5"
                      >
                         <div className="shrink-0 mt-1">
                            <Star className="w-5 h-5 text-[#D4AF37]" />
                         </div>
                         <p className="text-neutral-300 leading-relaxed text-sm md:text-base">{point}</p>
                      </motion.div>
                   ))}
                </div>
                ...
                <motion.div 
                  ...
                  className="text-center"
                >
                  <p className="inline-block px-6 py-3 bg-black text-neutral-400 text-sm rounded-full border border-white/5">
